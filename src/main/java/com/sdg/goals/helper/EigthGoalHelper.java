...
public class EigthGoalHelper {
	private static final Logger logger = LoggerFactory.getLogger(EigthGoalHelper.class);
	public static String TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
	 public static boolean hasExcelFormat(MultipartFile eighthGoalFile) {
		  
		    if (!TYPE.equals(eighthGoalFile.getContentType())) {
		      return false;
		    }

		    return true;
		  }
	  public static List<EigthGoal> excelToGoals(InputStream is,MultipartFile file) {
		    try {
			      ...
			      List<EigthGoal> goals = new ArrayList<EigthGoal>();
			      Row headerRow = sheet.getRow(0);
			        List<String> headerNames = new ArrayList<>();
				       ...
				       headerMap.put("contactnumber","Contact Number");
				        headerMap.put("informalemployment", "Informal Employment");
				        headerMap.put("hourlyearningsperday", "Hourly Earnings per day");
				        headerMap.put("isheshehavingbankaccount", "Is he/she having Bank Account?");
		...
		//
			        	 ...
			        	 case "informalemployment":
			        		 if(!(currentCell==null)) {
			        			 if(currentCell.getCellType()==CellType.STRING) {
			        		 goal.setInformal_Employment(currentCell.getStringCellValue());
							   if (!currentCell.getStringCellValue().equals("Employed")&&!currentCell.getStringCellValue().equals("Unemployed")) {
		                           cellValue = "Informal_Employment";
		                           errorMap.put(cellValue, "Invalid value, must be either 'Employed' or 'Unemployed', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		                       }}}
				        		else {
				        			goal.setInformal_Employment(null);
				        		}
			        		 break;
			        	 case "hourlyearningsperday":
			        		 if(!(currentCell==null)) {
			        			 if (currentCell.getCellType()==CellType.NUMERIC) {
			        		 goal.setHourly_Earnings_per_day((int)currentCell.getNumericCellValue());
			        			 }
			        			 else {
					        		  errorMap.put("Hourly Earnings per day", "Invalid value, must be a number , at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
								}
			        		 }
			        		 break;
			        	 case "isheshehavingbankaccount":
			        		 if(!(currentCell==null)) {
			        			 if(currentCell.getCellType()==CellType.STRING) {
			        		 goal.setIs_he_or_she_having_Bank_Account(currentCell.getStringCellValue());
							   if (!currentCell.getStringCellValue().equals("Yes")&& !currentCell.getStringCellValue().equals("No")) {
		                           cellValue = "Is_he_or_she_having_Bank_Account";
		                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
				        			...
				        			goal.setIs_he_or_she_having_Bank_Account(null);
				        		}
			        		 break;
			        	default:
				        	 break;
			      ...
			      if(!newAgeList.isEmpty()) {
			    	  logger.info("The following ages has been modified "+newAgeList.toString());
					      ...
					      if (!errorMap.isEmpty()) {
						     ...
						     logger.info("Eigth goal  sheet has exported to EigthGoalService successfully");

					   return goals;
				      }
				    catch (IOException e) {
				    	logger.error("fail to parse EigthGoals sheet",e);
					      throw new RuntimeException("fail to parse EigthGoals sheet: " + e.getMessage());
					    }
					  }
