...
public class SeventhGoalHelper {
	private static final Logger logger = LoggerFactory.getLogger(SeventhGoalHelper.class);

	public static String TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
	  public static boolean hasExcelFormat(MultipartFile seventhGoalFile) {
		  
		    if (!TYPE.equals(seventhGoalFile.getContentType())) {
		      return false;
		    }

		    return true;
		  }
	  public static List<SeventhGoal> excelToGoals(InputStream is,MultipartFile file) {
		    try {
		      ...
		      List<SeventhGoal> goals = new ArrayList<SeventhGoal>();
		      Row headerRow = sheet.getRow(0);
		        List<String> headerNames = new ArrayList<>();
			       ...
			       headerMap.put("contactnumber","Contact Number");
			        headerMap.put("accesstoelectricity", "Access to Electricity?");
			        headerMap.put("primaryuseofcleanfuelsandtechnology", "Primary use of Clean fuels and Technology?");
			        headerMap.put("renewableenergyusage", "Renewable energy usage?");
...
//		        Iterator<Cell> cellsInRow = currentRow.cellIterator();
		        ...
		        SeventhGoal goal = new SeventhGoal();
...
//
		        	...
		        	case "accesstoelectricity" :
		        		if(!(currentCell==null)) {
			        	   ...
			        	   goal.setAccess_to_Electricity(currentCell.getStringCellValue());
			        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
	                           cellValue = "Access_to_Electricity";
		        			...
		        			goal.setAccess_to_Electricity(null);
		        		}
			        	   break;
			        	case "primaryuseofcleanfuelsandtechnology" :
			        		if(!(currentCell==null)) {
			        			 if(currentCell.getCellType()==CellType.STRING) {
			        	   goal.setPrimary_use_of_Clean_fuels_and_Technology(currentCell.getStringCellValue());
			        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
	                           cellValue = "Primary_use_of_Clean_fuels_and_Technology";
	                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
	                       }}}
			        		else {
			        			goal.setPrimary_use_of_Clean_fuels_and_Technology(null);
			        		}
			        	   break;
			        	case "renewableenergyusage":
			        		if(!(currentCell==null)) {
			        			 if(currentCell.getCellType()==CellType.STRING) {
			        	   goal.setRenewable_energy_usage(currentCell.getStringCellValue());
			        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
	                           cellValue = "Renewable_energy_usage";
	                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
	                       }}}
			        		else {
			        			goal.setRenewable_energy_usage(null);
			        		}
			        	   break;
...
//		          cellIdx++;
		      ...
		      if (!errorMap.isEmpty()) {
			     ...
			     logger.info("Seventh goal  sheet has exported to SeventhGoalService successfully");

		   return goals;
		    } catch (IOException e) {
		    	logger.error("fail to parse SeventhGoals sheet",e);
