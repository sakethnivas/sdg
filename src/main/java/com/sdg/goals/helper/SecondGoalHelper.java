...
public class SecondGoalHelper {
	private static final Logger logger = LoggerFactory.getLogger(SecondGoalHelper.class);

	public static String TYPE = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
	  

	  public static boolean hasExcelFormat(MultipartFile secondGoalFile) {
		  
		    if (!TYPE.equals(secondGoalFile.getContentType())) {
		      return false;
		    }
		  ...
		  public static List<SecondGoal> excelToGoals(InputStream is,MultipartFile file) {
		    try {
		      ...
		      List<SecondGoal> goals = new ArrayList<SecondGoal>();
		      Row headerRow = sheet.getRow(0);
		        List<String> headerNames = new ArrayList<>();
			       ...
			       headerMap.put("undernourishcategory", "Undernourish Category?");
			       headerMap.put("prevalenceofundernourishment", "Prevalence of Undernourishment");
			       headerMap.put("height", "Height");
			       headerMap.put("weight", "Weight");
			       headerMap.put("stunted", "Stunted?");
			       headerMap.put("wasted", "Wasted?");
			       headerMap.put("overweight", "Over Weight?");
			       headerMap.put("productivity", "Productivity");
		           headerMap.put("plantbreedsstoragefacilityavailable", "Plant Breeds/Storage Facility Available?");
		      ...
		      while (rows.hasNext()) {
		        ...
		        SecondGoal goal = new SecondGoal();
...
//
		        	...
		        	case "undernourishcategory":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setUndernourish_Category(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes")&& !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Undernourish_Category";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		        			...
		        			goal.setUndernourish_Category(null);
		        		}
		        	   break;
		        	case "prevalenceofundernourishment":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setPrevalence_of_Undernourishment(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes")&&!currentCell.getStringCellValue().equals("NA") && !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Prevalence_of_Undernourishment";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No' or 'NA', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		        			...
		        			goal.setPrevalence_of_Undernourishment(null);
		        		}
		        	   break;
		        	case "height":
		        		if(!(currentCell==null)) {
		        	    ...
		        	    if(currentCell.getCellType() != CellType.NUMERIC) {
		        	        errorMap.put("Height", "Invalid value type, must be numeric, at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		        	        break;
		        	    }
		        	    goal.setHeight((int)currentCell.getNumericCellValue());
		        		}
		        		else {
		        			goal.setHeight(null);
		        		}
		        	...
		        	case "weight":
		        	    ...
		        	    if(currentCell.getCellType() != CellType.NUMERIC) {
		        	        errorMap.put("Weight", "Invalid value type, must be numeric, at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		        	        break;
		        	    }
		        	    goal.setWeight((int)currentCell.getNumericCellValue());
		        		}
		        		else {
		        			goal.setWeight(null);
		        		}
		        	...
		        	case "stunted":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setStunted(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Stunted";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
                       }
		        			...
		        			goal.setStunted(null);
		        		}
		        	   break;
		        	case "wasted":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setWasted(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Wasted";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
                       }}}
		        	...
		        	case "overweight":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setOver_Weight(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Over_Weight";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
                       }}}
		        		else {
		        			goal.setOver_Weight(null);
		        		}
		        	   break;
		        	case "productivity":
		        		if(!(currentCell==null)) {
		        	    if (currentCell.getCellType() != CellType.NUMERIC) {
		        	        cellValue = "Productivity";
		        	        errorMap.put(cellValue, "Invalid value type, must be a number, at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
		        	    } else {
		        	        goal.setProductivity((long)currentCell.getNumericCellValue());
		        	    }}
		        		else {
		        			goal.setProductivity(null);
		        		}
		        	    break;
		        	case "plantbreedsstoragefacilityavailable":
		        		if(!(currentCell==null)) {
		        			 if(currentCell.getCellType()==CellType.STRING) {
		        	   goal.setPlant_Breeds_Storage_Facility_Available(currentCell.getStringCellValue());
		        	   if (!currentCell.getStringCellValue().equals("Yes") && !currentCell.getStringCellValue().equals("No")) {
                           cellValue = "Plant_Breeds_Storage_Facility_Available";
                           errorMap.put(cellValue, "Invalid value, must be either 'Yes' or 'No', at row "+(currentCell.getRowIndex()+1)+" in ["+file.getOriginalFilename()+"]");
                       }}}
		        		else {
		        			goal.setPlant_Breeds_Storage_Facility_Available(null);
		        		}
		        	   break;
		      ...
		      if (!errorMap.isEmpty()) {
			     ...
			     logger.info("Secondgoal  sheet has exported to SecondGoalService successfully");

		   return goals;
		    } catch (IOException e) {
		    	logger.error("fail to parse SecondGoals sheet",e);
